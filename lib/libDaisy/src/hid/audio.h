...
class AudioHandle
{
  public:
    ...
    */
    ...
    /** Initializes audio to run using a single SAI, configured either in
     ** Stereo I2S mode or in TDM mode (SaiHandle::Config::tdm_slots > 0). */
    Result Init(const Config& config, SaiHandle sai);

    /** Initializes audio to run using two SAI, each configured independently
     ** in Stereo I2S or TDM mode. The first SAI drives the audio callback. */
     ...
     **
     ** Each initialized SAI contributes its TDM slot count if TDM is enabled
     ** (Config::tdm_slots > 0), otherwise 2 (stereo I2S).
     ** e.g. one stereo SAI = 2, two stereo SAI = 4, one 8-slot TDM SAI = 8,
     ** stereo SAI + 4-slot TDM SAI = 6.
