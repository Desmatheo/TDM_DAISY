...
class SaiHandle
{
  public:
    ...
    struct Config
    {
        ...
        /** Frame-sync polarity on the FS pin, TDM mode only.
         ** ACTIVE_LOW matches the AK4619-style TDM frame used originally by
         ** this fork. ACTIVE_HIGH matches the i.MX SAI frame generated by the
         ** Teensy 4.x Audio Library (1-BCLK-wide FS pulse, asserted one bit
         ** before the first data bit). */
        enum class TdmFsPolarity
        {
            ACTIVE_LOW,
            ACTIVE_HIGH,
        };

        /** Selects which block taps FS/SCK from the external pins when BOTH
         ** blocks are configured as SLAVE (external TDM master scenario).
         ** That block runs asynchronous (clocked from its pins) and the other
         ** block is internally synchronized to it.
         ** On Daisy Seed/Patch, the SAI2 pins broken out on the header are
         ** the block-B signals: FS = D27/PG9 (SAI2_FS_B), SCK = D28/PA2
         ** (SAI2_SCK_B) -- hence the BLOCK_B default. */
        enum class ExtClockBlock
        {
            BLOCK_A,
            BLOCK_B,
        };
        ...
        TdmFsPolarity tdm_fs_polarity = TdmFsPolarity::ACTIVE_LOW;
        ExtClockBlock ext_clock_block = ExtClockBlock::BLOCK_B;
    };
